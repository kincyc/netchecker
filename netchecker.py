...
def test_speed(ssid):
    ...
    try:
        ...
        error_message = (
            "Error:" + str(e).split(":")[-1]
        )  # Simplify the error message if needed
        return {
            "date": now.strftime("%Y-%m-%d"),
            "time": now.strftime("%H:%M:%S"),
            "network": ssid[:16].ljust(16),
...
def format_results(results):
    formatted_result = (
        f"{results['date']:10}  {results['time']:8}  {results['network']}  "
...
def main(interval_seconds, silent_mode):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run speed tests at regular intervals."
    )
    parser.add_argument(
        "--interval",
        "-i",
        type=int,
        default=300,
        help="Interval in seconds between each test (default is 300).",
    )
    parser.add_argument(
        "--silent",
        "-s",
        action="store_true",
        help="Run in silent mode without printing results to the screen.",
    )
